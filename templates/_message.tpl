...
{capture name='infoBombTpl'}
    <li><span class="msgItemText"></span><span class="bomb">💣</span></li>
{/capture}
...
<div id="infoPlaceHolder"
     ...
     data-bomb-template="{$smarty.capture.infoBombTpl|escape}"
>
    {foreach $message as $row}
    {if $row@first}{$smarty.capture.infoTplStart}{/if}
        <li><span class="msgItemText">{$row.long|escape}</span>{if "coin"==$row.type|default:false}<span class="coin-animated"></span>{elseif "bomb"==$row.type|default:false}<span class="bomb">💣</span>{/if}</li>
